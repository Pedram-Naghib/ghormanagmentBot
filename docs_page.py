...
<section id="filters">
  ...
  <table>
    <tr><th>دستور</th><th>کار</th></tr>
    <tr><td><code>افزودن فیلتر [کلمه/عبارت]</code><br><small>(یا فرم قدیمی: <code>افزودن کلمه فیلتر [کلمه]</code>)</small></td><td>هر پیام عادی (نه از ادمین/مالک/VIP) که این کلمه یا عبارت را <b>به‌طور کامل</b> داشته باشد خودکار حذف می‌شود. می‌توان یک عبارت طولانی/چند‌کلمه‌ای هم داد.</td></tr>
    <tr><td><code>حذف فیلتر [کلمه/عبارت]</code><br><small>(یا فرم قدیمی: <code>حذف کلمه فیلتر [کلمه]</code>)</small></td><td>برداشتن آن کلمه از لیست</td></tr>
    <tr><td><code>لیست فیلتر</code><br><small>(یا: <code>لیست کلمات فیلتر</code>)</small></td><td>نمایش همه‌ی کلمات فیلترشده‌ی این گروه</td></tr>
  </table>
  <div class="note">
    <b>تطبیق فقط روی کلمهٔ کامل است، نه زیررشته:</b> اگر «خر» فیلتر باشد، فقط پیامی که دقیقاً کلمهٔ «خر» را دارد حذف می‌شود؛ کلمه‌ای مثل «خرگوش» که فقط شامل آن حروف است، دیگر حذف نمی‌شود.
  </div>
  <div class="note">
...
<section id="spam">
  ...
  <div class="note">
    اینجا هم مثل «کلمات فیلترشده» (بخش ۷)، تطبیق فقط روی <b>کلمهٔ کامل</b> است، نه هر کلمه‌ای که فقط شامل آن باشد.
  </div>
  <div class="note">
